...
import { render, fireEvent } from '@testing-library/react';     // shallowtest  render to real dom
import Button, { PatButtonProps } from './Button';

describe('Buttton', () => {
  it('should match snapshot', () => {
    const { asFragment } = render(<Button> Snapshot Button </Button>);
    expect(asFragment()).toMatchSnapshot();     // 之前run的标答 如果变了更新就好
  });

  it('should render default button', () => {
    const btnProps = {
      onClick: jest.fn(),   // fake function and trace calling function
    };
    const wrapper = render(<Button {...btnProps}>Default Button</Button>);
    const element = wrapper.queryByText('Default Button') as HTMLElement;     // queryByText return html element
    expect(element).toBeInTheDocument();      // 存在
    expect(element.tagName).toBe('BUTTON');   // 自动大写的哦
    // to equal compare object 
  ...
  it('should render correct button based on different props', () => {
    ...
    expect(btnPrimarySmallElement).toHaveClass('btn btn-primary btn-sm test');      // this is include could be not same (differenet order etc )
  ...
  it('should render disabled button', () => {
    ...
    expect(btnDisabledLinkProps.onClick).toHaveBeenCalledTimes(0);      // this is disabled so not triggered
